...
| M10 加固与部署 | 进行中 | Compose 的 Backend 会先迁移，Worker 为真实 Celery 消费者；CI 包含前后端检查、后端 mypy、镜像构建和响应式首页 E2E，导入任务中间状态与公开 API 枚举保持一致；已提供请求 ID、JSON 访问日志、Compose 日志轮转、拒绝仓库内目标的外部备份/恢复脚本与备份目标健康提示。待本机 Docker build、空目录部署、实际备份恢复与部署环境验收。 |
