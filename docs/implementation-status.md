...
| M3–M9 学习能力 | 进行中 | 导入、写作、词汇复习和资源学习进度已有 Owner 隔离的持久化 API 与本机 PostgreSQL 集成测试；待接入真实媒体、Provider 与完整学习闭环。 |
...
已具备运行证据：单 Owner 创建/登录/刷新/退出基础，PostgreSQL migration，Redis + Celery 状态链，URL SSRF 防护，磁盘预算规则，导入/写作/词汇/学习进度的 Owner 持久化接口与本机数据库集成测试，响应式壳、浅深主题、焦点与 Reduced Motion，前后端基础测试与构建。
